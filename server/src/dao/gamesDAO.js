...
class GamesDAO {
  ...
  static async getGameStores(reqBody) {
    ...
    if (websites[epicgamesIndex])
      Object.assign(storeSites, { epicgames: websites[epicgamesIndex] });

    const stores = await this.getGameFromRetailers(storeSites);

    wrapTryCatch(
      this.redis
        .hmset(`igdb:${id}:stores`, flattenItemStores(stores))
        ...
        .then(([steam, epicgames, gog]) => {
          ...
          if (epicgamesProduct) {
            stores = Object.assign(stores, {
              epicgames: { url: epicgamesUrl, ...epicgamesProduct },
            });
          }
  ...
  static async addRemoveWishlist(userId, gameId) {
    ...
    let { wishlistedBy } = (await this.Games.findOne(
      { _id },
      { projection: { wishlistedBy: 1 } }
    )) || { count: 0, users: [] };
  ...
  static async getSteamOffers(page = 0, count = 15, imageRes) {
    ...
    let res = await fetch(
      `https://store.steampowered.com/contenthub/querypaginated/specials/TopSellers/render/?query=&start=${start}&count=15&cc=US&l=english&v=4&tag=`
    );
  ...
  static async getHumbleOffers(requestPage = 1) {
    ...
    let res = await fetch(
      `https://www.humblebundle.com/store/api/search?sort=bestselling&filter=onsale&hmb_source=store_navbar&request=${
        requestPage + 1
      }&page=${page}`
    );
  ...
  static async getGogOffers(requestPage = 1) {
    ...
    let res = await fetch(
      `https://www.gog.com/games/ajax/filtered?mediaType=game&page=${requestPage}&price=discounted&sort=popularity`
    );
  ...
  static hgetAllHashes(keys) {
    return Promise.all(keys.map((key) => this.redis.hgetall(key)));
  }
  ...
  static async cacheOffersItem(store_name, game) {
    ...
    await wrapTryCatch(this.redis.hmset(`${store_name}:${flattened.id}`, flattened));
